/*
Radial basis function interpolator interface
...
namespace FlightDynamics
{
    ...
    {
        /// <summary>
        /// The one and only RBF model
        /// </summary>
        alglib.rbfmodel Model = null;

        /// <summary>
        /// Model parameters. See https://www.alglib.net/interpolation/fastrbf.php
        /// </summary>
        double Radius = double.NaN;
        int NLayers = -1;
        double LambdaReg = double.NaN;

        /// <summary>
        /// A single database record of conditions and their associated results
        /// </summary>
        public class Case
        {
        ...
        /// <summary>
        /// Condition names (currently unused except for its length)
        /// </summary>
        public List<string> ConditionNames = new List<string>();

        /// <summary>
        /// Result names (currently unused except for its length)
        /// </summary>
        public List<string> ResultNames = new List<string>();

        /// <summary>
        /// See Usage string
        /// </summary>
        /// <param name="fname">See 'Rbfi.Usage' for information
        /// </param>
        public Rbfi(string fname = null)
        {
            ParseFile(fname);
        }
        ...
        /// 
        /// </summary>
        /// <param name="fname">Name of CSV database file. If not specified, use 'database.txt'</param>
        public void ParseFile(string fname = null)
        { 
            if(fname == null)
            {
                fname = "database.txt";
            ...
            // get header info and number of records
            foreach (var line in txt)
            {
                var lin = (string)(line.Trim());
                ...
                try
                {
                    ...
                    if (numRecords == 0)
                    {
                        ConditionNames = lin.Split(',', StringSplitOptions.TrimEntries).ToList();
                    }
                    else if (numRecords == 1)
                    {
                        ResultNames = lin.Split(',', StringSplitOptions.TrimEntries).ToList();
                    }
                    else if (numRecords == 2)
                    {
                        var parms = lin.Split(',', StringSplitOptions.TrimEntries).ToList();

                        Radius = Convert.ToDouble(parms[0]);

                        NLayers = Convert.ToInt32(parms[1]);

                        LambdaReg = Convert.ToDouble(parms[2]);
                    }
                }
                catch
                {
                    throw new Exception($"Error in '{fname}' line {numRecords}");
                }
            ...
            double[,] xy = new double[numRecords,ConditionNames.Count + ResultNames.Count];

            alglib.rbfcreate(ConditionNames.Count, ResultNames.Count, out Model);
            ...
            foreach (var line in txt)
            {
                ...
                try
                {
                    ...
                    {
                        var recordContents = lin.Split(',', StringSplitOptions.TrimEntries).ToList();

                        int n = 0;
                        foreach(var record in recordContents)
                        {
                            xy[num, n++] = Convert.ToDouble(record.Trim());
                        }
                    }
                }
                catch
                {
                    throw new Exception($"Error in '{fname}' line {num}");
                }
            ...
            alglib.rbfsetalgohierarchical(Model, Radius, NLayers, LambdaReg);

            alglib.rbfreport rep;
            alglib.rbfbuildmodel(Model, out rep);
            if(rep.terminationtype != 1)
            {
                throw new Exception("Failed to create model. Code: {rep.terminationtype}");
            }
        }
        ...
        public List<double> Calc(List<double> conditions)
        {
            var results = new double[ResultNames.Count];
            alglib.rbfcalcbuf(Model, conditions.ToArray(), ref results);

            return results.ToList();
        }
        
        static public string Usage = @"Usage:

FlightDynamics [-rb<DatabaseFile>]

Where:
<DatabaseFile> is a CSV file as described in the Remarks section. If not specified, then 'database.txt' is used.
...
The input conditions would be things like the power plant outputs, the control surface positions, propeller pitches, landing gear position, masses, relative air velocity, air density, ambient and current surface temperatures, etc. The outputs would be the force, torque, and surface temperatures on parts of the airframe or on the whole airframe.
